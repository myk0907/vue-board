...
export default {
  methods: {
    pageHeaderClick(e) {
      const header_about_el = document.getElementById('page_project');
      var click_menu = e.target.id;
      
      if(click_menu == "page_home"){
        header_about_el.classList.remove('router-link-exact-active');
      } else if(click_menu == "page_board"){
        header_about_el.classList.remove('router-link-exact-active');
      }
    }
  }
